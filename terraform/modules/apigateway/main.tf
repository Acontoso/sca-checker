resource "aws_api_gateway_rest_api" "gateway_object" {
  ...
  description = "API Gateway to act as Web Layer to SCA API"
...
resource "aws_api_gateway_method" "proxy_method_lambda" {
  ...
  authorization_scopes = ["sca-api/ioc.lookup.all"]
}
...
resource "aws_api_gateway_deployment" "single_deployment" {
  ...
  depends_on = [
    aws_api_gateway_rest_api_policy.resource_based_policy_attach,
    aws_api_gateway_method.proxy_method_lambda,
